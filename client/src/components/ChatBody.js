...
const ChatBody = ({ activeChat }) => {
  ...
  useEffect(() => {
    const handlePrivateMessage = (data) => {
      console.log("CLIENT RECEIVED: 'receivePrivateMessage' with data:", data);

      const isForCurrentChat =
        (data.sender === currentUser && data.recipient === activeChat) ||
        (data.sender === activeChat && data.recipient === currentUser);
      
      console.log(`EVALUATING: Is for current chat? ${isForCurrentChat}. (Sender: ${data.sender}, Recipient: ${data.recipient}, CurrentUser: ${currentUser}, ActiveChat: ${activeChat})`);
  ...
  // ... baaki ka JSX code same hai ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
        <div className="flex-grow p-4 overflow-y-auto bg-gray-100">
            <ToastContainer position="top-right" />
            {messages.map((msg, index) => (
                <div key={index} className={`mb-4 flex ${msg.sender === currentUser ? 'justify-end' : 'justify-start'}`}>
        ...
        {/* Typing indicator ko simplicity ke liye hata diya hai, isko baad mein add kar sakte hain */}
    </div>
  );
};
